...
def nu_guide():
    ...
    if int(cho) != 3:
        ...
        console.print(_("请选择需要启用的Nuitka插件，[green]绿色[/green]为自动启用的插件，若保持默认则输入D，希望启用或关闭请输入相应序号，用半角逗号隔开"))
        printlist = []
    ...
    if int(output_categories) == 2:
        ...
        AutoDep.copy_path(os.getcwd()+"/"+os.path.splitext(os.path.basename(build_instance.entrance))[0]+".dist",build_instance.target_path)
        ...
        with open("temporary_pack/" + "make_config" + ".json", "w", encoding='utf-8') as f:
